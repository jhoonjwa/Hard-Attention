...
def print_model_parameters_and_modules(model):
    # Print all parameters related to attention
...
def generate_for_attention(args, qs, image_features, tokenizer, model, model_name, conv_mode, bbox, ratio, option=None):
    ...
    self_attn_layer = model.model.layers[1]
    #hook = self_attn_layer.register_forward_hook(modify_attention_mask_hook(input_ids, bbox, ratio))
    ...
    input_output_attentions = input_output.attentions
    ...
    return outputs, input_tokens, output_tokens, weights_matrix, autoregressive_mask, image_attentions, input_output_attentions
...
def visualize_avg_image_attention_all_instances(vision_encoder_name, image_attentions_list, pool_method, hidden_top_k, qoutput_tokens_len_list, save_dir, file_prefix_list):
    ...
    fig, axes = plt.subplots(num_pool_methods, num_attn_maps, figsize=(4, 4))
    # single row
    if len(pool_methods) == 1:
        for col, (image_attentions_fused_pooled, file_prefix) in enumerate(zip(image_attentions_fused_pooled_list, file_prefix_list)):
            ...
            file_prefix = file_prefix.capitalize()
            sns.heatmap(image_attentions_fused_pooled, ax=axes, cmap='viridis', xticklabels=False, yticklabels=False, norm=norm, cbar=False)
            axes.set_title(f"{file_prefix}")
    # multiple rows
    else:
...
def prepare_attention_visualization(input_ids, input_output_ids, output_attend_to_input, tokenizer):
    ...
    Args:
        ...
        input_output_ids (torch.LongTensor): (batch_size*1, input_output_sequence_len)
...
def run_llava_with_attention(args):
    ...
    for idx, data in tqdm(enumerate(dataset)):
        if idx % args.subset != 0 and args.input == '/home/jovyan/fileviewer/vqa-attention/llava/attention/data/gqa_flickr30k.json':
            continue
        bbox = data['bbox']
        ...
        output, input_tokens, output_tokens, weights_matrix, autoregressive_mask, image_attentions, input_output_attention = generate_for_attention(args, qs, image_features, tokenizer, model, model_name, args.conv_mode, bbox, ratio, option=options)            
        
        print('new output', output)
        image_attention_mask = torch.load(os.getenv("SAVE_IMAGE_ATTENTION_MASK_PATH"))
        #print(image_attention_mask)
        
        #if args.output_visualization_tensors:
        #attention_ratio = image_attentions.sum() / input_output_attention.sum()
        if True:
            output_visualization_tensors = {
                ...
                'input_output_attention': input_output_attention
            }
